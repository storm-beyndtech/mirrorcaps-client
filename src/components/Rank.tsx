...
const Rank: React.FC<RankProps> = ({ rank, imageSrc, moreInfo }) => {
  ...
  return (
    ...
    >
      <div className="w-30 h-30 p-7 bg-gray-900 rounded-3xl overflow-hidden transition-transform transform-gpu hover:scale-105">
        <img
          src={imageSrc}
          alt={rank}
          className={`w-full h-full object-cover grayscale hover:grayscale-0 ${
            user.rank === rank ? 'grayscale-0' : ''
          }`}
        />
      </div>
        ...
        <div className="px-3 py-2 flex flex-col gap-1">
          ...
          <p className="flex gap-3">
            ...
            <span className="amount">
              ${moreInfo.minimumDeposit.toLocaleString('en-US')}
            </span>
          </p>
          <p className="flex gap-3">
          ...
          <p className="flex gap-3">
            ...
            <span className="amount">
              ${moreInfo.referralDeposits.toLocaleString('en-US')}
            </span>
          </p>
          <p className="flex gap-3">
            <span className="font-bold">Bonus:</span>
            <span className="amount">
              ${moreInfo.bonus.toLocaleString('en-US')}
            </span>
          </p>
        </div>
      </div>
